int a = 3+4;
int *fnc(int a,char b, int c, double dddd);

int main(int argv, char argc){
    if(argv==1){
        int a,b,c=8;
        a = --fnc(6,7);
    }
}
int* fm(int a, char b, double c, const char d, float f, const long long g, int* ptr1, char* ptr2){
	//int a; int k;
	//if(a){
	//	int ak='a',*d=88,sdfd;
	//	if(345435){
	//		char amyLIao='l',ouo='v';
	//		if(jack){float dc;}
	//		if(amyLove){double haha;}
	//		while(s){
	//			if(s){
	//				int k;
	//				double array[4515][77]={dddfdf};
	//			}
	//		}
	//	}
	//}
	while(1){
		//int test;
		double grid[5][79898]={1,3,5}, solar[10]['a'], kkkk[7], rar[232432][234][343][858799]={{1},{3,5},{7,{0,777,{'a','f','k'},0}},{9}};
        //int a,b,c,d,d,fe,fe,fefe,dfg;
        a = --++++(b+c-d+f-"strdfdffddf"(1,2,3,4,5,65));
	}
	//while(sfdfd){
	//	break;
	//	continue;
	//	return;
	//	switch(	dfdffd){
	//		case 0:
	//		case 1:
	//		case '1':
	//		case 2:
	//			if(dfdd){
	//				float  var[13][56]={123,dfdfg,{1,2,3}};
	//				int print=5,far=0,zz=1.45356656,k;
	//			}
	//		case 4:
	//		default:
	//		default: 
	//			if(k){
	//				double fff;
	//			}
	//		default:
	//	}
	//}
	return 0;
}
